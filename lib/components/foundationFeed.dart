 ...
 _reset(){
  ...
  _saveForm() async{
    ...
    if (formKey.currentState!.validate()) {
      ...
      if (formKey.currentState!.validate()) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Adding Foundation')),
        );
        ...
        await AddToDB().addFoundation(title,image,author,date,paragraphs,links).then((feed) =>
        {
          if(feed.isSuccesful){
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("Feed Foundation")),
            ),
            _reset()
          }
        }
      ...
      if (formKey.currentState!.validate()) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Adding notification')),
        );
        ...
        await AddToDB().addNotification(title,image,author,date,paragraphs,links).then((feed) =>
        {
          if(feed.isSuccesful){
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("Feed Notification")),
            ),
            _reset()
          }
        }
